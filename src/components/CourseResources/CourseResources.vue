<template>
    <div>
        ...
        <va-card>
            ...
            <va-card-content style="text-align: left">
                <a-dropdown position="tr" trigger="hover" @select="handleSelectCourse">
                    ...
                    <template #content>
                        <a-doption v-for="(course, index) in courses" :key="index"
                            :value="index">{{ course.name }}</a-doption>
                    </template>
                </a-dropdown>
                <a-list size="small" :bordered="false" style="text-align: left; margin-top: 20px"
        ...
        <a-modal v-model:visible="uploadModalVisible">
            ...
            <el-upload ref="upload" :action="getUploadUrl()" :headers="headers()" :data="additionalData()"
                :auto-upload="false" style="text-align: center" :on-success="handleOnSuccess" :on-error="handleOnError">
                <template #trigger>
...
export default {
    ...
    methods: {
        ...
        generateUrl(fileName) {
            ...
            return this.$URL + "/file/download/courseResource/" + +this.courses[this.curCourseIndex].id + "/" + fileName;
        },
