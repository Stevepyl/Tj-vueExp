                ...
                <va-card v-for="(course, index) in courses" :key="index" color="#f2f3f5" gradient
                    ...
                    <va-card-content style="rgb(60, 60, 60); font-weight: bold">
                        <div style="display: flex">
                            ...
                            <div><va-button @click="switchToCourse(course.id, index)" color="#e0e5df"
                                    style="color: rgb(40,40,40)">点击进入</va-button></div>
                        </div>
                    </va-card-content>
                </va-card>
            </va-card-content>
...
export default {
    ...
    mounted() {
        ...
        fetch(this.$URL + "/manages/get/teacher/detail?teacherId=" + this.teacherId, {
            method: "GET",
