    ...
    <div>
        <div style="margin-bottom: 10px">
            <va-breadcrumbs separator=">">
                ...
                <va-breadcrumbs-item :label="this.Scorestatus == 1 ? '所有实验项目' : '学生综合成绩'" disabled />
            </va-breadcrumbs>
        </div>
        <va-card>
        ...
        this.Scorestatus == 1 ? "课程项目成绩" : "学生综合成绩"
            ...
            >
              <va-card-content v-if="this.Scorestatus==1" style="rgb(60, 60, 60); font-weight: bold">
                ...
                <el-table v-if="this.Scorestatus == 1" :data="
    tasks.filter(
        (data) =>
            !search ||
...
" style="width: 100%">
                    ...
                    <el-table-column align="right">
                        ...
                        <template #default="scope">
                            <a-button @click="switchToTask(scope.row)">点击进入</a-button>
                        </template>
                    </el-table-column>
                </el-table>

                <el-table v-if="this.Scorestatus == 2" stripe :data="
    student.filter(
        (data) =>
            !search ||
            data.name.toLowerCase().includes(search.toLowerCase())
...
export default {
    ...
    mounted() {
        ...
        fetch(this.$URL + "/takes/get/course/detail?courseId=" + this.courseId, {
            ...
            .then(async () => {
                fetch(this.$URL + "/takes/get/course?courseId=" + this.courseId, {
                    method: "GET",
                    headers: { satoken: localStorage.getItem("token") },
                })
                    .then((res) => res.json())
                    .then((res) => {
                        ...
                        for (let i = 0; i < this.student.length; ++i) {
                            this.student[i].score =
                                res[i].score != null ? res[i].score : "暂无分数";
                        }
                    });
            });
    },
    methods: {
        handleMark(row) {
            fetch(
                this.$URL +
                "/finishes/get/record/detail/studentAndCourse?courseId=" +
                this.courseId +
                "&studentId=" +
                row.id,
                {
