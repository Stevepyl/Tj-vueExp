<template>
    <div>
        ...
        <!-- 以下是新增公告的Modal -->
        <a-modal v-model:visible="modalVisible" footer="false" hide-cancel ok-text="取消">
            <template #title>
                新增公告
            </template>
            <a-form :model="form">
                ...
                <a-form-item field="content" label="详细内容" :rules="[{ required: true, message: '请输入公告内容' }]">
                    <a-textarea v-model="form.content" :auto-size="{ minRows: 3, maxRows: 8 }" placeholder="输入公告内容"
                        allow-clear />
                </a-form-item>
                <a-form-item>
        ...
        <a-modal v-model:visible="displayAnnouncementModalVisible" hide-cancel ok-text="关闭"
            ...
            <a-textarea style="margin-bottom: 10px; background: white" v-show="!modifyAnnouncementStatus"
                v-model="this.displayAnnouncement.content" auto-size readonly />
        ...
        <!-- 以下是确认移除公告的Modal -->
        <a-modal v-model:visible="removeModalVisible" simple v-on:ok="handleConfirmRemove">
            <template #title>
                移除公告
            </template>
        ...
        <!-- 激活账号的modal -->
        <a-modal v-model:visible="activateAccountModalVisible" hide-cancel ok-text="取消"
