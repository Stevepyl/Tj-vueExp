<template>
    <div>
        ...
        <!-- 修改信息的Modal -->
        <a-modal v-model:visible="modifyInfoModalVisible" hide-cancel ok-text="取消" v-on:close="handleCloseModifyModal">
        ...
        <!-- 移除用户的提醒Modal -->
        <a-modal v-model:visible="removeModalVisible" v-on:ok="confirmRemove" simple>
            ...
            <va-card-content>
                <div style="display: flex">
                    ...
                    <el-upload :action="url" :http-request="importExcel" list-type="text" :show-file-list="false">
                        <a-button type="primary" style="background: #496f8d">导入用户</a-button>
                    </el-upload>
                </div>
                <el-table v-show="this.status == 1" stripe :data="
    students.filter(
        (data) =>
...
">
                ...
                <el-table v-show="this.status == 2" stripe :data="
    teachers.filter(
        (data) =>
            !search | data.name.toLowerCase().includes(search.toLowerCase())
    )
">
                ...
                <el-table v-show="this.status == 3" stripe :data="
    allUsers.filter(
        (data) =>
            !search | data.name.toLowerCase().includes(search.toLowerCase())
    )
