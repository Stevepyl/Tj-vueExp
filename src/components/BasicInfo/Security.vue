<template>
    <div>
        ...
        <va-card>
            ...
            <va-card-title style="font-size: 20px; font-weight: bold">账号安全</va-card-title>
            <va-card-content>
                ...
                <div style="text-align: left">
                    ...
                    <div style="display: flex; line-height: 200%">
                        ...
                        <a-form ref="formRef" :model="passwords">
                            ...
                            <a-form-item field="confirmPassword" label="请确认新密码：" :rules="rules">
...
export default {
    ...
    methods: {
        handleSendEmail() {
            // this.step = 2
            this.loading1 = true
            fetch(this.$URL + "/user/updatePassword/sendEmail?userId=" + this.userId, {
                method: "POST"
            })
                .then(res => res.json())
                .then(res => {
        ...
        handleSubmitVerificationCode() {
            // this.step = 3
            this.loading2 = true
            fetch(this.$URL + "/user/updatePassword/checkCode?code=" + this.verificationCode + "&userId=" + this.userId, {
                method: "POST"
            })
                .then(res => res.json())
                .then(res => {
        ...
        handleSubmitNewPassword() {
            if (this.passwords.password == undefined || this.passwords.password == '') {
                this.$message.error("请输入新密码")
                return
            } else if (this.passwords.confirmPassword == undefined || this.passwords.confirmPassword == '') {
                this.$message.error("请输入新密码")
            } else if (this.passwords.password != this.passwords.confirmPassword) {
                this.$message.error("两次输入的密码不一致")
            } else {
                fetch(this.$URL + '/user/updatePassword?userId=' + this.userId + "&newPassword=" + this.passwords.password, {
                    method: "POST",
