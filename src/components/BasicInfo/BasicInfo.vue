<template>
    <div>
        ...
        <va-modal ref="modal" v-model="showAvatar" hide-default-actions title="查看大图">
            <div><img style="max-height: 450px" :src="myAvatarUrl()" /></div>
            <div style="text-align: center">
                <va-button flat style="margin-top: 15px; color: gray"
                    @click="this.showAvatar = !this.showAvatar">关闭</va-button>
            </div>
        ...
        <a-modal v-model:visible="activateAccountModalVisible" hide-cancel ok-text="取消"
            ...
            <div v-show="!loading">
                ...
                <div style="text-align: center" v-show="isSendingVerificationEmailButtonClicked">
                    <a-alert type="success">验证码已发送至：{{ this.email }}<br>请在30分钟内进行填写，逾期验证码将失效。</a-alert>
                ...
                <table class="va-table">
                    <tbody style="font-size: 16px">
                        <tr>
                            ...
                            <td>
                                <va-avatar square :src="myAvatarUrl()" @click="this.showAvatar = !this.showAvatar"
...
export default {
    ...
    methods: {
        ...
        handleSendVerificationEmail() {
            ...
            fetch(this.$URL + "/user/verify/send?email=" + this.email + "&userId=" + this.id, {
                method: "POST",
                headers: { "satoken": localStorage.getItem("token") }
            })
                .then(res => res.json())
        ...
        handleSubmitVerificationCode() {
            ...
            fetch(this.$URL + "/user/verify/activate?code=" + this.verificationCode + "&userId=" + this.id, {
                method: "POST",
                headers: { "satoken": localStorage.getItem("token") }
            })
                .then(res => {
